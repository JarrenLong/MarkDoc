...
class MarkDoc {
  ...
  protected function generateTOC() {
    $baseDir = getcwd() . "/";
    $allFiles = $this->getDirContents($baseDir, $mdFile);
    ...
    foreach($allFiles as &$p) {
	  ...
	  $tmp = str_replace($mdFile,'',$tmp);
      fwrite($toc, ' * [' . $tmp . '](http://' . $_SERVER["SERVER_NAME"] . '?' . $requestVar . '=' . $tmp . ")\n");
    }
  ...
  protected function importFromWP($wpHost, $wpUser, $wpPass, $wpDbName) {
    ...
    if($res->num_rows > 0) {
      while($row = $res->fetch_assoc()) {
        $fn = "posts/" . str_replace(" ", "_", $row["post_title"]) . $mdFile;
        $contents = "# " .
            $row["post_title"] .
            "\n\n### " .
            $row["posted_by"] .
  ...
  public function processRequest() {
    ...
    $page = htmlspecialchars($_GET[$requestVar]);
